...
const DailyForecast: React.FC<DailyForecastProps> = ({ items, unit }) => {
  ...
  // Helper to convert Kelvin to Celsius/Fahrenheit
  const convertTemp = (kelvin: number) =>
    unit === 'C' ? kelvin - 273.15 : (kelvin - 273.15) * 9 / 5 + 32;

  // Group 3h forecasts by date
  ...
  const dailySummaries: DailyData[] = Object.entries(grouped).map(([date, dayItems]) => {
    ...
    // Pick description and icon from the first item (or use majority later)
  ...
  return (
    <div>
      ...
      {/* Show 3h forecast for selected day */}
      {selectedDate && (
        <ForecastHourlyList
